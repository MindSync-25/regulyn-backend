...
@Component
public class AdobeStubEsignProvider implements EsignProvider {

    private final ObjectMapper objectMapper;

    public AdobeStubEsignProvider(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    @Override
    public ProviderId getProviderId() {
        return ProviderId.ADOBE;
    }
    ...
    public EsignCreateRequestResult createSigningRequest(EsignCreateRequest cmd) {
        ...
        String providerEnvelopeId = "ad_env_" + envelopeHash;
        String signingUrl = "https://adobe-sign.example/sign/" + providerEnvelopeId;
    ...
    public EsignWebhookParseResult parseAndVerifyWebhook(EsignWebhookRequest req) {
        ...
        if (providerEventId == null || providerEventId.isBlank()) {
            providerEventId = "ad_evt_" + payloadSha256.substring(0, 16);
        }
        if (providerEnvelopeId == null || providerEnvelopeId.isBlank()) {
            providerEnvelopeId = "ad_env_" + payloadSha256.substring(0, 24);
        }
