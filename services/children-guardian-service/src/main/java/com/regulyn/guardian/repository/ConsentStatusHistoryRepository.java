package com.regulyn.guardian.repository;

import com.regulyn.guardian.entity.ConsentStatusHistory;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.UUID;

@Repository
public interface ConsentStatusHistoryRepository extends JpaRepository<ConsentStatusHistory, UUID> {
    
    List<ConsentStatusHistory> findByConsentIdOrderByChangedAtDesc(UUID consentId);
    
    List<ConsentStatusHistory> findByConsentIdOrderByChangedAtAsc(UUID consentId);
}
