...
import com.regulyn.guardian.config.TestApiKeyConfig;
...
@Testcontainers
@Import({TestSecurityConfig.class, TestApiKeyConfig.class, TestPermitAllSecurityConfig.class})
class EsignIntegrationTest {
    ...
    void esignRequestIsIdempotentAndAudited() throws Exception {
        ...
        CreateEsignRequestRequest request = new CreateEsignRequestRequest(
                "STUB",
                "GUARDIAN_CONSENT",
                1,
                "https://return.local"
        ...
        GuardianConsent reloaded = consentRepository
                .findByTenantIdAndConsentId(tenantId, consent.getConsentId())
                .orElseThrow();
    ...
    void webhookSignatureFailureDoesNotStoreArtifact() throws Exception {
        ...
        Integer failedCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM children.esign_webhook_events " +
                        "WHERE tenant_id = ? AND signature_verification_status = 'FAILED'",
                Integer.class,
                tenantId
        );
    ...
    void webhookVerifiedStoresArtifactAndUpdatesConsent() throws Exception {
        ...
        Integer verifiedCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM children.esign_webhook_events " +
                        "WHERE tenant_id = ? AND signature_verification_status = 'VERIFIED'",
                Integer.class,
                tenantId
        );
        ...
        assertThat(consentRepository
                .findByTenantIdAndConsentId(tenantId, consent.getConsentId())
                .orElseThrow()
                .getSignedArtifactId()).isNotNull();
    ...
    void webhookIsIdempotent() throws Exception {
        ...
        Integer webhookCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM children.esign_webhook_events " +
                        "WHERE tenant_id = ? AND provider_envelope_id = ? AND provider_event_id = ?",
                Integer.class,
                tenantId,
                esignResponse.providerEnvelopeId(),
                "evt-4"
    ...
    private CreateEsignRequestResponse createEsignRequest(
            UUID tenantId,
            UUID userId,
